...
async def handle_function(bot: BaseBot, ev: BaseEvent, msg: UniMsg, session: Uninfo):
    ...
    async for source, seg in iter_sources_in_message(msg):
        if save_img:
            ...
            if (
                check_res.label in config.nailong_mute_seconds.keys()
                and config.nailong_mute_seconds[check_res.label] > 0
            ):
                functions.append(
                    lambda: mute(bot, ev, config.nailong_mute_seconds[check_res.label]),
                )
